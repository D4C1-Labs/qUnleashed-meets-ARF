...
bool hitag2_hell_recover(
    ...
    // were never evaluated inside a chunk. That stalled the reported progress
    // (only the between-chunk leader push moved, which freezes for tens of
    // seconds on a heavy chunk) and delayed cancellation. We now checkpoint
