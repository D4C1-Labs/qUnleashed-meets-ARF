...
abstract class UniversalBleTransportBase extends Transport {
  ...
  Future<void> configureConnected(String deviceId) async {
    ...
    // Ask for the lowest-latency connection interval (Android:
    // CONNECTION_PRIORITY_HIGH, ~7.5-15 ms vs the ~30-50 ms default). This is
    // the single biggest lever for remote-desktop responsiveness: screen
    // frames (multiple notifications) and input write-with-response ACKs are
    // both gated on the connection interval, so a slow default interval makes
    // the app trail the Flipper by hundreds of ms to seconds. Best-effort:
    // unsupported platforms just throw and are ignored.
    try {
      await _ops.requestHighPriority(deviceId).timeout(_gattOpTimeout);
      Log.info('[BLE] requested high-priority connection interval');
    } catch (e) {
      Log.info('[BLE] requestHighPriority not applied: $e');
    }
  ...
  Future<void> _drainBudgetCycle() async {
    ...
    while (isActive && cycleRemaining > 0) {
      if (_txQueue.isEmpty) {
        // Only linger long enough to coalesce a burst that is already being
        // enqueued (e.g. the PRESS/SHORT/RELEASE trio of one button, queued
        // back-to-back within microseconds). The old 100 ms wait added that
        // full delay to every interactive input because a single button's
        // frames drain the queue and then park here waiting for the *next*
        // user action that only comes seconds later. A short window keeps the
        // full-MTU coalescing win for real bursts without stalling lone
        // inputs, which is the remote-desktop lag the user reported.
        final signal = _txDataSignal = Completer<void>();
